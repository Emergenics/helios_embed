// --- START OF FILE src/helios_embed/incremental_nystrom_engine_cpu.cpp (Formatted) ---
...
IncrementalNystromEngine::IncrementalNystromEngine(torch::Tensor landmarks,
  ...
  TORCH_CHECK(
      false,
      "Helios.Embed was compiled in CPU-only mode. The 'IncrementalNystromEngine' "
...
torch::Tensor IncrementalNystromEngine::update(
    const torch::Tensor& X_new_in, const torch::Tensor& Phi_old_in) {
  return torch::Tensor();
}
// --- END OF FILE src/helios_embed/incremental_nystrom_engine_cpu.cpp (Formatted) ---