// --- START OF FILE src/helios_embed/nystrom_engine.cu (FINAL v1.2.0 with NVTX) ---
#include "nystrom_engine.h"
#include <ATen/ATen.h>
#include <nvtx3/nvToolsExt.h> // The NVTX header for profiling ranges

// ... (validate_inputs_stateless function is unchanged) ...
void validate_inputs_stateless(const at::Tensor& X, const at::Tensor& Lm) {
    ...
    TORCH_CHECK(Lm.size(0) > 0, "Landmarks tensor must have at least one landmark (m > 0).");
...
torch::Tensor compute_rkhs_embedding(
    ...
    float ridge)
{
    // --- NVTX PROFILING RANGE ---
    nvtxRangePushA("Helios.Embed::compute_rkhs_embedding");
    ...
    if (X.size(0) == 0) {
        nvtxRangePop(); // Pop the range before returning
    ...
    // --- Sub-range for kernel matrix computation ---
    nvtxRangePushA("K_nm_and_K_mm_computation");
    auto K_nm = at::exp(-gamma * at::cdist(X, Lm).pow(2));
    auto K_mm = at::exp(-gamma * at::cdist(Lm, Lm).pow(2));
    nvtxRangePop();

    // --- Sub-range for stable inverse sqrt ---
    nvtxRangePushA("stable_inverse_sqrt");
    ...
    // --- Sub-range for final GEMM ---
    nvtxRangePushA("final_gemm");
    auto result = at::mm(K_nm, K_mm_inv_sqrt).contiguous();
    nvtxRangePop();

    nvtxRangePop(); // Pop the main range
    return result;
}
// --- END OF FILE src/helios_embed/nystrom_engine.cu (FINAL v1.2.0 with NVTX) ---