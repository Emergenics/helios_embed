// --- START OF FILE src/helios_embed/nystrom_engine.cu (FINAL, STABLE v1.1.0) ---
    ...
    float ridge)
{
    ...
    // --- SECURITY: Enforce contiguous memory layout right at the boundary ---
    auto X = X_in.contiguous();
    auto Lm = landmarks_in.contiguous();
    
    // --- Gracefully handle empty input edge case ---
    if (X.size(0) == 0) {
        return torch::empty({0, Lm.size(0)}, X.options());
    }

    // --- SECURITY: The Hardened Input Validation Gate ---
    ...
    // Core, validated logic using numerically stable eigen-decomposition.
    ...
    // Use eigh for superior numerical stability over Cholesky for this step.
    auto eigh_result = at::linalg_eigh(K_mm_reg, "L");
    auto eigenvalues = std::get<0>(eigh_result);
    auto eigenvectors = std::get<1>(eigh_result);

    // Clamp eigenvalues for stability, preventing division by zero.
...
// --- END OF FILE src/helios_embed/nystrom_engine.cu (FINAL, STABLE v1.1.0) ---